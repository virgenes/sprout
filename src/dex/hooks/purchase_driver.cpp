...
void deliver_complete(pvz2_elf_image_t *img, GuestRuntime *rt, std::uint32_t thiz,
    ...
    if (fn == 0) {
        LOG_IAP("cannot grant \"%s\": FirePaymentComplete not registered",
                    sku.c_str());
        return;
    }
    ...
    if (g_receipt == 0 || g_base64 == 0 || g_sku == 0 || g_json == 0 || g_sig == 0 || g_order == 0) {
        LOG_IAP("cannot grant \"%s\": guest heap exhausted", sku.c_str());
        return;
    }
    ...
    LOG_IAP("granting \"%s\" -> FirePaymentComplete(fn=0x%08x, ptr=0x%08x)",
...
void deliver_refresh(pvz2_elf_image_t *img, GuestRuntime *rt, std::uint32_t thiz,
    ...
    if (fn == 0) {
        LOG_IAP("FireDidRefresh not registered -- refresh dropped");
        return;
    }
    ...
    if (arr == 0 || (std::uint64_t)arr + bytes > img->mem_size) {
        LOG_IAP("cannot build SkuDetails array -- heap exhausted");
        return;
    }
    ...
    LOG_IAP("FireDidRefresh(fn=0x%08x, ptr=0x%08x, %u sku(s))", fn, native_ptr, n);
    runtime::call_guest_between_frames_n(fn, args, (int)(sizeof(args) / sizeof(args[0])));
}
...
void iap_deliver_pending(pvz2_elf_image_t *img, GuestRuntime *rt) {
    ...
    if (!ptr_known || thiz == 0) {
        LOG_IAP("%zu action(s) pending but driver/ptr unknown -- dropping",
                    due.size());
        return;
    }
