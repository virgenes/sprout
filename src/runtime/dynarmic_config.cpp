...
void join_leftover_guest_threads(GuestRuntime *rt) {
    /* Signal every blocking primitive so threads parked in sem_wait /
     * pthread_cond_wait can observe the shutdown flag and halt. */
    rt->shutdown = true;
    {
        std::lock_guard<std::mutex> lk(rt->sems_lock);
        for (auto &kv : rt->guest_sems) kv.second->cv.notify_all();
    }
    {
        std::lock_guard<std::mutex> lk(rt->conds_lock);
        for (auto &kv : rt->guest_conds) kv.second->cv.notify_all();
    }
...
void run_guest_call(pvz2_elf_image_t *img, GuestRuntime *rt, const char *label,
    ...
    if (!per_frame) set_status("%s", label);
