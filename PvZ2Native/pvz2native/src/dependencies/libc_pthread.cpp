...
void c_cond_wait(GuestCall &c) {
    ...
    gm->unlock(self);
    auto pred = [&] { return gc->generation != gen || c.rt->shutdown; };
    if (!gc->cv.wait_for(lk, kStuckWarnAfter, pred)) {
...
void c_sem_wait(GuestCall &c) {
    ...
    gs->waiters++;
    auto pred = [&] { return gs->count > 0 || c.rt->shutdown; };
    if (!gs->cv.wait_for(lk, kStuckWarnAfter, pred)) {
        lk.unlock();
...
void c_sem_timedwait(GuestCall &c) {
    ...
    std::unique_lock<std::mutex> lk(gs->m);
    auto pred = [&] { return gs->count > 0 || c.rt->shutdown; };
    if (gs->cv.wait_until(lk, deadline, pred)) {
        if (c.rt->shutdown) {
            c.halt("shutdown");
            return;
        }
