...
bool RsbIndex::load(const std::string &obb_path) {
    ...
    if (!f) {
        LOG_VFS("[rsb-index] cannot open '%s'", obb_path.c_str());
        return false;
    }
    ...
    if (std::fread(head.data(), 1, head.size(), f) != head.size() ||
        ...
        LOG_VFS("'%s' is not an RSB", obb_path.c_str());
        std::fclose(f);
        return false;
    }
    ...
    if (rsg_number == 0 || rsg_info_each < 0x88) {
        LOG_VFS("implausible RSG table (n=%u each=%u)", rsg_number, rsg_info_each);
        std::fclose(f);
        return false;
    }
    ...
    if (std::fseek(f, (long)rsg_info_begin, SEEK_SET) != 0 ||
        ...
        LOG_VFS("truncated RSG info table");
        std::fclose(f);
        return false;
    }
    ...
    LOG_VFS("%zu files indexed from %u RSGs (%u skipped) in '%s'",
