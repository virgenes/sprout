#include <sprout/audio/sound_repository.h>
...
static sprout_sound_t sprout_sound_repository[] = {
    {
        .name = "random.pop2",
        .resources_count = 1,
        .resources = {
...
sprout_sound_resource_t *sprout_get_sound_buffer(char *name) {
    for (int i = 0; sprout_sound_repository[i].name != NULL; ++i) {
        sprout_sound_t *sound = &sprout_sound_repository[i];
        if (strcmp(sound->name, name) == 0) {
            srand(time(NULL));
            int resource_index = rand() % sound->resources_count;
            sprout_sound_resource_t *resource = &sound->resources[resource_index];
            if (resource->buffer == NULL) {
                ...
                if (samples_count != -1) {
                    ...
                    memset(resource, 0, sizeof(sprout_sound_resource_t));
                }
            }
            return resource;
        }
