...
void audio_engine_init() {
    if (!audio_engine_initialized) {
        ...
        if (SDL_Init(SDL_INIT_AUDIO) < 0) {
            LOG_AUDIO("SDL_Init Audio failed: %s", SDL_GetError());
            return;
        }
        ...
        if (!audio_engine_device) {
            LOG_AUDIO("SDL_OpenAudioDevice failed: %s", SDL_GetError());
            return;
        }
...
void audio_engine_tick() {
    ...
    for (int i = 0; i < AUDIO_ENGINE_MAX_STREAMS; ++i) {
        if (audio_engine_streams[i].buffer) {
            total++;
            if (audio_engine_streams[i].active) active++;
        }
    }
    SDL_UnlockAudioDevice(audio_engine_device);
    LOG_AUDIO("[engine] %d/%d streams active", active, total);
}
