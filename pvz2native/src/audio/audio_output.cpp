...
void SDLCALL audio_callback(void * /*user*/, Uint8 *stream, int len) {
    ...
    while (written < len) {
        ...
        std::vector<std::uint8_t> *buf = g_slots[slot].data.get();
        if (buf == nullptr) {
            break;
        }
...
bool configure(int channels, int sample_rate_hz, int bits_per_sample) {
    std::lock_guard<std::mutex> ctl(g_control_mutex);
     ...
     * those contain data that will never be consumed. */
    {
        ...
        for (std::uint32_t i = cs; i < ws; ++i) {
            g_slots[i & kSlotMask].data.reset();
        }
    }
...
bool enqueue(const void *pcm, std::size_t bytes) {
    ...
    auto &s = g_slots[slot];
    if (s.data == nullptr) {
        s.data = std::make_unique<std::vector<std::uint8_t>>(p, p + bytes);
    } else {
...
void shutdown() {
    ...
    {
        std::lock_guard<std::mutex> cv(g_cv_mutex);
        g_shutdown = true;
    }
    g_completion_cv.notify_all();
