...
void get_paths(){
    ...
    _snprintf_s(g_gamePath,sizeof(g_gamePath),_TRUNCATE,"%s\\pvz2native.exe",g_exeDir);
...
void read_controls(void){
    ...
    while(fgets(line,sizeof(line),f)){
        ...
        *eq='\0';char*k=t,*v=eq+1;
        ...
        int iv=atoi(v);
        for(int i=0;i<NUM_BINDS;i++){
            if(strcmp(k,g_binds[i].key)==0){g_binds[i].kb=iv;break;}
        }
        if(strncmp(k,"gp_",3)==0){
            for(int i=0;i<NUM_BINDS;i++){
                if(strcmp(k+3,g_binds[i].key)==0){g_binds[i].gp=iv;break;}
            }
        }
    }
    fclose(f);
